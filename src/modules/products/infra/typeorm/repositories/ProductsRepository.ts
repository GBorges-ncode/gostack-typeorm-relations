...
class ProductsRepository implements IProductsRepository {
  ...
  public async findAllById(products: IFindProducts[]): Promise<Product[]> {
    ...
    const productList = await this.ormRepository.find({ id: In(productId) });

    if (productId.length !== productList.length) {
      throw new AppError('An product is missing');
    }
  ...
  public async updateQuantity(
    ...
    const newProducts = productsData.map(productData => {
      ...
      const productTemp = productData;

      productTemp.quantity -= productFind.quantity;
